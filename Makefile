...
# Variables
GO := go
BINARY_NAME := gtfs-netex-converter
BUILD_DIR := bin
...
build: ## Build the CLI binary
	...
	@mkdir -p $(BUILD_DIR)
	@$(GO) build -ldflags="$(LDFLAGS)" -o $(BUILD_DIR)/$(BINARY_NAME) ./cmd/gtfs-netex-converter
	@echo "Built: $(BUILD_DIR)/$(BINARY_NAME)"

build-all: ## Build binaries for all platforms
	@echo "Building for all platforms..."
	@mkdir -p $(BUILD_DIR)
	@GOOS=linux GOARCH=amd64 $(GO) build -ldflags="$(LDFLAGS)" -o $(BUILD_DIR)/$(BINARY_NAME)-linux-amd64 ./cmd/gtfs-netex-converter
	@GOOS=linux GOARCH=arm64 $(GO) build -ldflags="$(LDFLAGS)" -o $(BUILD_DIR)/$(BINARY_NAME)-linux-arm64 ./cmd/gtfs-netex-converter
	@GOOS=windows GOARCH=amd64 $(GO) build -ldflags="$(LDFLAGS)" -o $(BUILD_DIR)/$(BINARY_NAME)-windows-amd64.exe ./cmd/gtfs-netex-converter
	@GOOS=darwin GOARCH=amd64 $(GO) build -ldflags="$(LDFLAGS)" -o $(BUILD_DIR)/$(BINARY_NAME)-darwin-amd64 ./cmd/gtfs-netex-converter
	@GOOS=darwin GOARCH=arm64 $(GO) build -ldflags="$(LDFLAGS)" -o $(BUILD_DIR)/$(BINARY_NAME)-darwin-arm64 ./cmd/gtfs-netex-converter
	@echo "Built all platform binaries in $(BUILD_DIR)/"

install: ## Install the CLI binary
	@echo "Installing $(BINARY_NAME)..."
	@$(GO) install -ldflags="$(LDFLAGS)" ./cmd/gtfs-netex-converter
...
docs: .FORCE ## Generate documentation
	...
	@$(GO) doc -all ./cmd/gtfs-netex-converter > docs/package.txt 2>/dev/null || echo "No main package documentation available" > docs/package.txt
	@echo "Generating HTML documentation (if godoc is available)..."
	@which godoc >/dev/null 2>&1 && (godoc -http=:6060 & sleep 3 && curl -s http://localhost:6060/pkg/github.com/theoremus-urban-solutions/gtfs-netex-converter/ > docs/api/index.html && pkill -f "godoc -http=:6060") || echo "godoc not found - skipping HTML generation"
...
docs-serve: ## Start documentation server
	...
	@echo "Visit: http://localhost:6060/pkg/github.com/theoremus-urban-solutions/gtfs-netex-converter/"
	@godoc -http=:6060
...
docker-build: ## Build Docker image
	...
	@docker build -t gtfs-netex-converter:$(VERSION) .
