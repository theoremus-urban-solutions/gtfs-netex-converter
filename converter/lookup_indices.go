...
// Uses pointers to avoid data duplication and reduce memory usage
...
// Uses pointers to avoid duplicating data in memory
func BuildLookupIndices(data *GTFSData) *LookupIndices {
	...
	// Index stop times by trip ID (using pointers to avoid duplication)
	for i := range data.StopTimes {
		st := &data.StopTimes[i]
		indices.stopTimesByTripID[st.TripID] = append(indices.stopTimesByTripID[st.TripID], st)
	}

	// Index trips by route ID (using pointers to avoid duplication)
	...
	// Index calendar dates by service ID (using pointers to avoid duplication)
	...
	// Index transfers by stop ID (using pointers to avoid duplication)
	...
	// Index shapes by shape ID (using pointers to avoid duplication)
...
// Returns pointers to reduce memory copies
